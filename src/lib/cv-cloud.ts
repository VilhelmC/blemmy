...
export const OAUTH_POST_MESSAGE_TYPE = 'cv-supabase-oauth-done';
...
function authDebugEnabled(): boolean {
	...
	try {
		return new URLSearchParams(window.location.search).get('cv-auth-debug') === '1';
	} catch {
		return false;
	}
}
...
function authDebugLog(message: string, extra?: unknown): void {
	...
	if (extra !== undefined) {
		console.info('[cv-auth]', message, extra);
	} else {
		console.info('[cv-auth]', message);
	}
}
...
function appendAuthDiagnosticsDetails(panel: HTMLElement, lines: string[]): void {
	...
	det.className = 'cv-oauth-overlay__details';
	const sum = document.createElement('summary');
	sum.textContent = 'Auth diagnostics (for debugging)';
	det.appendChild(sum);
	const pre = document.createElement('pre');
	pre.className = 'cv-oauth-overlay__debug';
...
function detachOauthPopupShell(): void {
	document.documentElement.classList.remove('cv-oauth-popup-shell');
}
...
function attachOAuthOverlayDismiss(overlay: HTMLElement, panel: HTMLElement): void {
	...
	row.className = 'cv-oauth-overlay__actions';
	const btn = document.createElement('button');
	btn.type = 'button';
	btn.className = 'cv-oauth-overlay__btn';
...
export function bootstrapOAuthFromUrl(): void {
	...
	overlay.className = 'cv-oauth-overlay';
	if (isOAuthCallbackPopupWindow()) {
		overlay.classList.add('cv-oauth-overlay--oauth-popup');
		document.documentElement.classList.add('cv-oauth-popup-shell');
	}
	const panel = document.createElement('div');
	panel.className = 'cv-oauth-overlay__panel';
	const text = document.createElement('p');
	text.className = 'cv-oauth-overlay__text';
...
export async function listDocuments(): Promise<CloudResult<CloudDocument[]>> {
	try {
		...
		const latestByDoc = new Map<string, CVData>();
...
export async function createDocument(
	...
	try {
		...
		const { data: doc, error: docErr } = await sb
			.from('documents')
			.insert({ name, doc_type: docType, user_id: user.id })
			.select()
			.single();
...
export async function loadVersion(versionId: string): Promise<CloudResult<CVData>> {
	try {
		const { data, error } = await getClient()
			.from('document_versions')
			.select('data')
...
const ACTIVE_DOC_KEY = 'cv-cloud-active-doc';

 ...
 *   - System prompt summary builder (for blemmy-chat-prompts.ts)
 */
 ...
 *  - work[N]        → nth `.experience-block` (work row)
 *  - work[N].highlights[M] → specific <li> within that work item
 *  - education[N]   → nth `.edu-block` within `#blemmy-education`
 *  - skills.*       → `#blemmy-rebalance-skills` (skills section root)
...
export function resolvePathToElement(path: ContentPath): HTMLElement | null {
	...
	if (path.startsWith('skills')) {
		return document.getElementById('blemmy-rebalance-skills')
	...
	if (path.startsWith('layout') || path.startsWith('style')) {
		return document.getElementById('blemmy-prefs-trigger');
	}
 ...
 * Called by blemmy-chat-prompts.ts when CVData.review is present.
 */

...
const STORAGE_KEY = 'cv-chat-config';
...
export function loadChatConfigMeta(): LoadChatConfigMeta {
	try {
		const local = parseStoredChat(localStorage.getItem(STORAGE_KEY));
		if (local) {
			const okL = normalizeChatConfig(local);
			if (okL) { return { config: okL, source: 'local' }; }
			try { localStorage.removeItem(STORAGE_KEY); } catch { /* ignore */ }
		}
	} catch { /* ignore */ }
	try {
		const sess = parseStoredChat(sessionStorage.getItem(STORAGE_KEY));
		if (sess) {
			const okS = normalizeChatConfig(sess);
			if (okS) { return { config: okS, source: 'session' }; }

 ...
 *
 * Pipeline:
 *  1. PROFILE      Measure section width-sensitivity (layout-profile.ts).
 *  2. GENERATE     Produce constraint-valid LayoutCandidates (layout-candidate.ts).
 ...
 *                  emit blemmy-alternatives-ready if multiple peaks found.
 ...
 *
 * Preferences (from blemmy-prefs.ts) are read on init and on blemmy-prefs-changed.
 * A selected alternative (from blemmy-alternative-selected) re-enters the pipeline
...
} from '@lib/engine/layout-candidate';
...
const SLACK_VAR_P1_MAIN          = '--blemmy-slack-gap-p1-main';
const SLACK_VAR_P1_SIDEBAR       = '--blemmy-slack-gap-p1-sidebar';
const SLACK_VAR_P2_MAIN          = '--blemmy-slack-gap-p2-main';
const SLACK_VAR_P2_SIDEBAR       = '--blemmy-slack-gap-p2-sidebar';
const SLACK_VAR_P1_MAIN_INNER    = '--blemmy-slack-gap-p1-main-inner';
const SLACK_VAR_P1_SIDEBAR_INNER = '--blemmy-slack-gap-p1-sidebar-inner';
const SLACK_VAR_P2_MAIN_INNER    = '--blemmy-slack-gap-p2-main-inner';
const SLACK_VAR_P2_SIDEBAR_INNER = '--blemmy-slack-gap-p2-sidebar-inner';
const ALIGN_VAR_P1_SIDEBAR       = '--blemmy-align-gap-p1-sidebar';
const ALIGN_VAR_P1_MAIN          = '--blemmy-align-gap-p1-main';
const ALIGN_VAR_P2_SIDEBAR       = '--blemmy-align-gap-p2-sidebar';
const ALIGN_VAR_P2_MAIN          = '--blemmy-align-gap-p2-main';
const SIDEBAR_WIDTH_VAR          = '--blemmy-sidebar-width-override';

const LOG = '[layout-engine]';
...
function clearSidebarWidth(card: HTMLElement): void {
	...
	delete card.dataset.blemmyLayoutSidebarMm;
	invalidateIntrinsicHeightCache();
}
...
function portraitHeaderBlock(els: LayoutEls): HTMLElement | null {
	const hb = els.portraitCell?.querySelector('.blemmy-header-block');
...
function restoreMastheadToFull(els: LayoutEls): void {
	...
	if (!masthead) { return; }
	const mastZoneId = spec.mastheadId;
	if (mastZoneId) {
		const zoneEl = document.getElementById(mastZoneId);
		if (zoneEl) { delete zoneEl.dataset.blemmyVariant; }
	}
...
function applyMastheadMode(els: LayoutEls, mode: string): void {
	const defaultMode = spec.zoneVariants
		? Object.values(spec.zoneVariants)[0]?.[0] ?? 'full'
		: 'full';
	if (!mode || mode === defaultMode) { restoreMastheadToFull(els); return; }

	const zoneId = spec.mastheadId;
	const zoneEl = zoneId ? document.getElementById(zoneId) : null;
	if (zoneEl) { zoneEl.dataset.blemmyVariant = mode; }
	...
	const profileCol = spec.profileColId ? document.getElementById(spec.profileColId) : null;
	if (hb == null) { return; }
	if ((mode === 'compact' || mode === 'profile-sidebar-meta') && mastheadRight != null) {
		hb.appendChild(mastheadRight);
		invalidateIntrinsicHeightCache();
		return;
	}
	if ((mode === 'minimal' || mode === 'profile-main') && elProfile != null &&
		profileCol?.contains(elProfile)) {
...
function setFooterCols(footer: HTMLElement | null, cols: 0 | 1 | 2 | 3): void {
	if (!footer) { return; }
	if (cols === 0) { footer.style.removeProperty('--blemmy-footer-cols'); }
	else            { footer.style.setProperty('--blemmy-footer-cols', String(cols)); }
	invalidateIntrinsicHeightCache();
}
...
function readIntrinsicGridHeightPx(page: HTMLElement): number {
	...
	const grid = page.querySelector('.blemmy-grid');
	if (!grid) { return 0; }
	page.classList.add('blemmy-layout-measure-intrinsic');
	void page.offsetHeight;
	const h = (grid as HTMLElement).scrollHeight;
	page.classList.remove('blemmy-layout-measure-intrinsic');
...
function readP2ContentSums(els: LayoutEls): { sbSum: number; mainSum: number } {
	...
	const grid = page2.querySelector('.blemmy-grid') as HTMLElement | null;
	page2.classList.add('blemmy-layout-measure-intrinsic');
	...
	page2.classList.remove('blemmy-layout-measure-intrinsic');
...
function clearSlackVars(els: LayoutEls): void {
	...
	els.card.removeAttribute('data-blemmy-layout-slack-fill');
	invalidateIntrinsicHeightCache();
}
...
function clearAlignVars(els: LayoutEls): void {
	...
	for (const c of cols) { for (const v of vars) { c.style.removeProperty(v); } }
	els.card.removeAttribute('data-blemmy-layout-align-applied');
	invalidateIntrinsicHeightCache();
}
...
async function applySlackDistribution(els: LayoutEls): Promise<void> {
	...
	if (applied.length > 0) { els.card.dataset.blemmyLayoutSlackFill = applied.join(','); }
	await raf2();
...
async function applyAlignmentPass(els: LayoutEls): Promise<void> {
	...
	if (notes.length > 0) {
		...
		if (applied) { els.card.dataset.blemmyLayoutAlignApplied = summary; }
...
async function finaliseLayout(
	...
	if (isSingle) {
		...
		for (let f = 1; f <= 3; f++) {
			...
			els.card.classList.add('blemmy-fill-' + f);
			await raf2();
			if (readIntrinsicGridHeightPx(els.page1) > MAX_SINGLE_PAGE_PX) {
				els.card.classList.remove('blemmy-fill-' + f); break;
			}
			best = f;
		}
		els.card.classList.remove(...[1,2,3].map((n) => spec.fillClassPrefix + n));
		if (best > 0) { els.card.classList.add('blemmy-fill-' + best); }
		els.card.dataset.blemmyLayoutFill = String(best);
	}
...
export function initLayoutEngine(documentSpec: EngineDocumentSpec): () => void {
	...
	function widthSignature(): string {
		const grid = els.page1.querySelector('.blemmy-grid') as HTMLElement | null;
		/*
	...
	function layoutTargetActive(): boolean {
		return (
			els.shell.classList.contains('blemmy-print-preview') ||
	...
	function setStatus(msg: string, type: 'ok' | 'warn'): void {
		...
		els.statusEl.className   = 'blemmy-layout-status no-print blemmy-status-' + type;
	}

	function markReady(): void {
		els.card.setAttribute('data-blemmy-layout-ready', 'true');
	...
	async function applyAlternative(candidateId: string): Promise<void> {
		...
		els.card.removeAttribute('data-blemmy-layout-ready');
		els.card.dataset.blemmyLayoutMeasuring = 'true';
		fullReset(els);
		clearSidebarWidth(els.card);
		clearSlackVars(els);
		clearAlignVars(els);
		...
		delete els.card.dataset.blemmyLayoutMeasuring;
		markReady();
	...
	async function applyLayout(): Promise<void> {
		...
		const prevPages = Number(els.card.dataset.blemmyLayoutPages ?? 0);
		const prevWinnerId = els.card.dataset.blemmyLayoutWinnerId ?? '';
		const editModeActive = document.documentElement.classList.contains('blemmy-edit-mode');
		...
		els.card.removeAttribute('data-blemmy-layout-ready');
		els.card.dataset.blemmyLayoutMeasuring = 'true';
		els.card.removeAttribute('data-blemmy-layout-error');
		els.card.dataset.blemmyLayoutPagePreference = prefs.pagePreference;
		els.card.removeAttribute('data-blemmy-layout-winner-id');
		els.card.removeAttribute('data-blemmy-layout-candidates-total');
		els.card.removeAttribute('data-blemmy-layout-candidates-single');
		els.card.removeAttribute('data-blemmy-layout-candidates-two');
		els.card.removeAttribute('data-blemmy-layout-scored-total');
		els.card.removeAttribute('data-blemmy-layout-scored-single');
		els.card.removeAttribute('data-blemmy-layout-scored-two');
		els.card.removeAttribute('data-blemmy-layout-ms');
		fullReset(els);
		clearSidebarWidth(els.card);
		clearSlackVars(els);
		clearAlignVars(els);
		els.card.classList.remove(...[1,2,3].map((n) => spec.fillClassPrefix + n));
		els.card.dataset.blemmyLayoutFill = '0';
		...
		if (!layoutTargetActive()) {
			...
			els.card.dataset.blemmyLayoutDisposition = 'web-idle';
			els.card.dataset.blemmyLayoutPages       = '2';
			els.card.dataset.blemmyLayoutMs    = String(Math.round(performance.now() - t0));
		...
		const profiles = profileAllSections(
			...
			els.sidebar1,
			spec.alwaysSidebarIds[0] ?? 'blemmy-education',
		);
		...
		els.card.dataset.blemmyLayoutWorkItemsCount = String(workDiag.count);
		els.card.dataset.blemmyLayoutWorkItemsSource = workDiag.source;
		...
		const candidates = generateCandidates(
			workCount,
			profiles,
			spec.movableSections,
			spec.alwaysSidebarIds,
			spec.zoneVariants,
			spec.multiPageDefaultVariants,
			candDiag,
		);
		...
		els.card.dataset.blemmyLayoutCandidatesTotal  = String(candidates.length);
		els.card.dataset.blemmyLayoutCandidatesTried = String(candDiag.totalCombinations);
		els.card.dataset.blemmyLayoutCandidatesRejected = String(candDiag.rejected);
		const topRejection = Object.entries(candDiag.rejectionsByRule)
			.sort((a, b) => b[1] - a[1])[0];
		if (topRejection) {
			els.card.dataset.blemmyLayoutCandidatesRejectTop = `${topRejection[0]}:${topRejection[1]}`;
		} else {
			els.card.removeAttribute('data-blemmy-layout-candidates-reject-top');
		}
		...
		els.card.dataset.blemmyLayoutCandidatesSingle = String(single1.length);
		els.card.dataset.blemmyLayoutCandidatesTwo = String(two2.length);
		...
		if (allScored.length === 0) {
			els.card.dataset.blemmyLayoutScoredTotal = '0';
			els.card.dataset.blemmyLayoutScoredSingle = '0';
			els.card.dataset.blemmyLayoutScoredTwo = '0';
			els.card.dataset.blemmyLayoutError = 'no-valid-candidate';
			els.card.dataset.blemmyLayoutMs = String(Math.round(performance.now() - t0));
		...
		els.card.dataset.blemmyLayoutScoredTotal  = String(allScored.length);
		els.card.dataset.blemmyLayoutScoredSingle = String(allScored.filter((s) => s.candidate.pages === 1).length);
		els.card.dataset.blemmyLayoutScoredTwo    = String(allScored.filter((s) => s.candidate.pages === 2).length);
		...
		els.card.dataset.blemmyLayoutWinnerId = winner.candidate.id;
		...
		if (isSingle) {
			...
			setDensity(els.card, chosenDensity);
			els.card.dataset.blemmyLayoutDensity = String(chosenDensity);
		}
		...
		els.card.dataset.blemmyLayoutDisposition  = isSingle ? 'single-page' : 'two-page';
		els.card.dataset.blemmyLayoutPages        = isSingle ? '1' : '2';
		els.card.dataset.blemmyLayoutHeaderVariant = winner.candidate.mastheadMode;
		els.card.dataset.blemmyLayoutSections = JSON.stringify(winner.candidate.sections);
		els.card.dataset.blemmyLayoutWorkSplit        = String(winner.candidate.pageSplitWork);
		if (nearOfl) { els.card.dataset.blemmyLayoutOverflowRisk = 'true'; }
		else         { els.card.removeAttribute('data-blemmy-layout-overflow-risk'); }
		...
		layoutAuditLog('engine-apply:done', {
			winnerId: winner.candidate.id,
			layoutMs: els.card.dataset.blemmyLayoutMs,
			pages: els.card.dataset.blemmyLayoutPages,
		});
...
/** @deprecated Use initLayoutEngine. */
export const initCvLayoutEngine = initLayoutEngine;

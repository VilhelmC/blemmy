 ...
 *
 * These four values feed directly into layout-candidate.ts:
...
export type SectionId =
	| 'blemmy-rebalance-skills'
	| 'blemmy-rebalance-languages'
	| 'blemmy-rebalance-interests'
	| 'blemmy-rebalance-profile'
	| 'blemmy-education'       // synthetic id for the education div in sidebar-1
 ...
 *                         Defaults to 'blemmy-education' for backwards compatibility.
 *
...
export function profileAllSections(
	...
	alwaysSidebarId: SectionId = 'blemmy-education',
): SectionProfileMap {
	...
	const alwaysEl = Array.from(sidebar1El.children).find(
		(c): c is HTMLElement =>
			...
			!c.classList.contains('blemmy-sidebar-tail-spacer'),
	) ?? null;
	if (alwaysEl) {
...
export function logProfiles(profiles: SectionProfileMap): void {
	...
	for (const [id, p] of profiles) {
		rows.push({
			section:       id.replace('blemmy-rebalance-', '').replace('blemmy-', ''),
	...
	console.groupCollapsed('[layout-profile] Section profiles');
	console.table(rows);
	console.groupEnd();
}

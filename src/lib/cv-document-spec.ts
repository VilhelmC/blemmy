 ...
 * section in cv.doctype.json and the corresponding IDs in cv-renderer.ts.
 */
...
if (!_spec) {
	throw new Error('[cv-document-spec] cv.doctype.json not registered — check document-type.ts');
}

/**
 * The EngineDocumentSpec for the CV document type.
 * Passed to initCvLayoutEngine() on startup.
 *

 ...
 *
 * The engine (cv-layout-engine.ts) owns all DOM mutations and reflow
 ...
 *   C6  mastheadMode 'full' is the only mode on two-page candidates
 *       (masthead strip modes are single-page rescue paths only)
 *
 ...
 *   S4  Single-page: mastheadMode other than 'full' slightly penalised
 *       (prefer full masthead; rescue modes are last resort)
 */
...
/**
 * Approximate combined horizontal padding inside .cv-main (print mode).
 ...
 * ~1.15rem ≈ 5mm at 10pt root (matches gap in #cv-page-*-body-footer).
 */
 ...
 * Accounts for horizontal padding inside .cv-main and gaps between columns.
 */
...
function hardConstraintViolation(
	...
	// C6: masthead rescue modes only on single-page
	if (pages === 2 && mastheadMode !== 'full') {
		return 'C6: masthead rescue modes only valid on single-page';
	}
 ...
 *
 * @param workCount   Total number of work entries in cv-content.json
...
export function generateCandidates(
	...
	const mastheadModes1:     MastheadMode[]     = ['full', 'profile-sidebar-meta', 'profile-main', 'classic'];
	const mastheadModes2:     MastheadMode[]     = ['full'];
...
export function widthAffinityPenalty(
	...
	// Masthead rescue penalty (soft preference for full masthead)
	if (candidate.mastheadMode !== 'full') {
...
export function clusterLabel(s: ScoredCandidate): string {
	...
	if (c.mastheadMode !== 'full') {
		const mhLabels: Record<string, string> = {
			'profile-sidebar-meta': 'slim header',
			'profile-main':         'minimal header',
			'classic':              'no header band',
		};

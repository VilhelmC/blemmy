 ...
 *                   Persisted to localStorage under 'blemmy-edit-draft'.
...
import { clearLegacyPortraitStorage } from '@lib/profile-data-loader';
...
function applyFieldEdit(
	...
	// skills.<category>.N — category keys match validateSkills in blemmy-loader
...
export function exportAsJson(data: CVData): void {
	...
	a.href      = url;
	a.download  = 'blemmy-content.json';
...
function enableWorkItemDrag(
	...
	marker.id = 'blemmy-work-insert-marker';
	...
	function activateDraggable(): void {
		shell.querySelectorAll<HTMLElement>('[data-blemmy-drag-idx]').forEach((el) => {
			el.draggable = false;
			if (el.querySelector('.blemmy-drag-handle')) { return; }
	...
	return () => {
		...
		obs.disconnect();
		marker.remove();
		shell.querySelectorAll('.blemmy-drag-handle').forEach((el) => el.remove());
	};
}
...
function enablePortraitReplacement(
	...
	const imgEl = document.getElementById('blemmy-portrait-img') as HTMLImageElement | null;
...
function injectTagEditors(
	...
	function renderTagRow(
		...
		function rebuildChips(): void {
			...
			const input = row.querySelector('.blemmy-tag-input-wrap');
			row.innerHTML = '';
	...
	document.querySelectorAll<HTMLElement>('[data-blemmy-drag-idx]').forEach((el) => {
		...
		const row = renderTagRow(
			...
			(tag) => {
				...
				const existingRow = el.querySelector('.blemmy-tag-row');
				if (existingRow) {
	...
	return () => {
		...
		injected.length = 0;
		document.getElementById('blemmy-tag-datalist')?.remove();
	};
}
...
function buildSidePanel(
	...
	const panel = h('div', {
		id: 'blemmy-edit-panel',
		class: `blemmy-edit-panel blemmy-side-panel ${DOCKED_SIDE_PANEL_CLASS} no-print`,
	});

	// Header
	panel.appendChild(h('div', { class: 'blemmy-edit-panel__header' },
		h('span', { class: 'blemmy-edit-panel__title' }, 'Hidden content'),
		h('span', { class: 'blemmy-edit-panel__hint' }, 'Click ↩ to restore'),
	));

	const list = h('div', { class: 'blemmy-edit-panel__list' });

	// Hidden work items
	for (const idx of hiddenWork) {
		const entry = data.work[idx];
		if (!entry) { continue; }
		const tile = h('div', { class: 'blemmy-edit-panel__tile', 'data-panel-type': 'work' },
			h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Work'),
			h('span', { class: 'blemmy-edit-panel__tile-label' }, entry.company),
			h('span', { class: 'blemmy-edit-panel__tile-sub' }, entry.position),
		);
		const restoreBtn = h('button', {
			class: 'blemmy-edit-panel__restore',
			type:  'button',
			title: 'Restore to CV',
		}, '↩');
	...
	for (const wKey of Object.keys(hiddenHl)) {
		...
		for (const hi of idxList) {
			...
			const tile = h('div', {
				class: 'blemmy-edit-panel__tile',
				'data-panel-type': 'work-highlight',
			},
			h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Bullet'),
			h('span', { class: 'blemmy-edit-panel__tile-label' }, entry.company),
			h('span', { class: 'blemmy-edit-panel__tile-sub' }, short || `Index ${hi}`),
			);
			const restoreBtn = h('button', {
				class: 'blemmy-edit-panel__restore',
				type:  'button',
	...
	for (const idx of hiddenEdu) {
		...
		h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Education'),
		h('span', { class: 'blemmy-edit-panel__tile-label' }, entry.institution),
		h('span', { class: 'blemmy-edit-panel__tile-sub' }, entry.degree),
		);
		const restoreBtn = h('button', {
			class: 'blemmy-edit-panel__restore',
			type:  'button',
	...
	for (const eKey of Object.keys(hiddenEduHl)) {
		...
		for (const hi of idxList) {
			...
			h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Edu bullet'),
			h('span', { class: 'blemmy-edit-panel__tile-label' }, entry.institution),
			h('span', { class: 'blemmy-edit-panel__tile-sub' }, short || `Index ${hi}`),
			);
			const restoreBtn = h('button', {
	...
	for (const cat of Object.keys(hiddenSk)) {
		...
		for (const si of idxList) {
			...
			h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Skill'),
			h('span', { class: 'blemmy-edit-panel__tile-label' }, `${cat}: ${label}`),
			);
			const restoreBtn = h('button', {
				class: 'blemmy-edit-panel__restore',
				type:  'button',
	...
	for (const li of hiddenLangs) {
		...
		h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Language'),
		h('span', { class: 'blemmy-edit-panel__tile-label' }, lang.language),
		h('span', { class: 'blemmy-edit-panel__tile-sub' }, lang.fluency),
		);
		const restoreBtn = h('button', {
			class: 'blemmy-edit-panel__restore',
			type:  'button',
	...
	for (const id of hiddenSects) {
		const tile = h('div', { class: 'blemmy-edit-panel__tile', 'data-panel-type': 'section' },
			h('span', { class: 'blemmy-edit-panel__tile-badge' }, 'Section'),
			h('span', { class: 'blemmy-edit-panel__tile-label' }, sectionLabels[id] ?? id),
		);
		const restoreBtn = h('button', {
			class: 'blemmy-edit-panel__restore',
			type:  'button',
	...
	if (list.children.length === 0) {
		list.appendChild(h('p', { class: 'blemmy-edit-panel__empty' },
...
function injectReorderControls(
	...
	const eduSectionEl = document.getElementById('blemmy-education');
	if (eduSectionEl instanceof HTMLElement && !hiddenSects.includes('education')) {
		injectPair(
			eduSectionEl,
			'blemmy-move-controls--section',
			{
				hide: {
	...
	const profileEl = document.getElementById('blemmy-rebalance-profile');
...
export function activateEditMode(
	...
	const shellEl = document.getElementById(BLEMMY_DOC_SHELL_ID);
	if (!shellEl) {
		throw new Error(`[blemmy-editor] #${BLEMMY_DOC_SHELL_ID} not found`);
	}
	const shell = shellEl;
	...
	document.documentElement.classList.add('blemmy-edit-mode', 'blemmy-edit-mode');
	...
	refreshPanel();
	window.dispatchEvent(new CustomEvent('blemmy-view-mode-changed'));
	...
	function deactivate(): void {
		...
		document.documentElement.classList.remove('blemmy-edit-mode', 'blemmy-edit-mode');
		window.dispatchEvent(new CustomEvent('blemmy-view-mode-changed'));
	}
